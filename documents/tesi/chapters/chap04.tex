...
Il primo passo è stato di scrivere un \textit{Software Development Plan}, un documento scritto per impostare le prime linee guida sulla progettazione di \textcolor{red}{NOME}.
...
\begin{table}[h]
    ...
    38-39                                                           & \begin{tabular}[c]{@{}l@{}}Design del progetto\\ (diagrammi e requisiti)\end{tabular}          & \textit{Software Requirements and Design} \\
...
\begin{figure}[h]
    \centering
    \includegraphics[width=16cm]{images/gantt2.png}
    \caption{Diagramma di Gantt dello sviluppo del serious game}
\end{figure}

\newpage
\section{Analisi dei requisiti}
Un passo fondamentale delle prime fasi è stato individuare i requisiti funzionali e non funzionali del progetto \textcolor{red}{NOME}.

Durante lo sviluppo del progetto questi requisiti, divisi tra funzionali e non funzionali, sono stati raccolti nel documento \textit{Software Requirements and Design} 
per mantenere un approccio ordinato e scalabile, da un progetto sviluppato singolarmente ad uno progettato con un team.

In questo capitolo verranno identificati gli attori del sistema, definiti i requisiti funzionali e non funzionali, e descritti i casi d'uso principali.

\subsection{Attori del sistema}
Per comprendere meglio le esigenze degli utenti, sono state identificate due tipologie principali di attori:
\begin{enumerate}
    \item \textbf{Insegnante}: Vuole un'interfaccia chiara, la possibilità di modificare domande esistenti senza dover riscrivere tutto, e la capacità di salvare i quiz su chiavetta USB per portarli in aula. Non ha competenze di programmazione.
    \item \textbf{Studente}: Vuole un feedback immediato sulle risposte (giusto/sbagliato), conoscere il punteggio finale e avere una sfida stimolante (es. la sfida giornaliera).
\end{enumerate}

\subsection{Requisiti funzionali}
I requisiti funzionali descrivono ciò che il sistema deve fare. Sono stati divisi in base alle aree di funzionalità.

\subsubsection{Modalità di gioco}
\begin{itemize}
    \item L'utente deve poter selezione e giocare a un quiz a sua scelta;
    \item Ogni giorno deve esserci un nuovo quiz giornaliero, generato automaticamente dal sistema;
    \item Deve essere salvata e mostrata la serie di quiz giornalieri completati con successo;
    \item Il gioco deve mostrare un feedback positivo in caso di risposta corretta;
    \item Il gioco deve mostrare un feedback negativo in caso di risposta sbagliata, facendo in modo che l’utente non venga disincentivato dal continuare il quiz.
\end{itemize}

\subsubsection{Gestione dei quiz}
\begin{itemize}
    \item L'utente deve poter creare un nuovo quiz;
    \item L'utente deve poter modificare un quiz esistente;
    \item L'utente deve poter eliminare un quiz esistente;
    \item L'utente deve poter aggiungere domande a un quiz esistente;
    \item L'utente deve poter modificare le domande di un quiz esistente;
    \item L'utente deve poter rimuovere domande da un quiz esistente;
    \item L'utente deve poter visualizzare la lista dei quiz esistenti;
    \item L'utente deve poter importare e esportare un quiz da un file esterno;
\end{itemize}

\subsection{Requisiti non funzionali}
I requisiti non funzionali individuati sono stati:
\begin{itemize}
    \item Usabilità: L'interfaccia utente deve essere intuitiva. Il tempo di apprendimento per un nuovo utente deve essere inferiore ai 5 minuti.
    \item \textit{Performance}: Il gioco deve essere responsivo, dall’azione dell’utente alla risposta deve passare meno di 1 secondi (nessun lag percettibile);
    \item Portabilità dei dati: I file dei quiz devono essere in formato JSON, per essere leggibili e modificabili esternamente;
    \item Portabilità del gioco: Il gioco deve essere compatibili su diverse piattaforme;
\end{itemize}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=8cm]{images/Use_case1.drawio.png}
...
\begin{table}[H]
    \begin{tabular}{@{}llccl@{}}
    \toprule
    \textbf{Nome}                                                                 & \textbf{Descrizione}                                                                                        & \textbf{Probabilità} & \textbf{Pericolosità} & \textbf{Prevenzione}                                                                                      \\ \midrule
    \textit{Scope creep}                                                          & \begin{tabular}[c]{@{}l@{}}Provare ad\\ implementare\\ troppi requisiti\end{tabular}                        & Media                & Alta                  & \begin{tabular}[c]{@{}l@{}}Requisiti ben\\ definiti e chiara\\ pianificazione\\ degli sprint\end{tabular} \\
                                                                                &                                                                                                             & \multicolumn{1}{l}{} & \multicolumn{1}{l}{}  &                                                                                                           \\
    \begin{tabular}[c]{@{}l@{}}Ritardi di \\ sviluppo\end{tabular}                & \begin{tabular}[c]{@{}l@{}}Ritardi di sviluppo\\ rispetto al\\ piano scelto\end{tabular}                    & Media                & Media                 & \begin{tabular}[c]{@{}l@{}}Definizione di\\ obiettivi intermedi\\ raggiungibili\end{tabular}              \\
                                                                                &                                                                                                             & \multicolumn{1}{l}{} & \multicolumn{1}{l}{}  &                                                                                                           \\
    \begin{tabular}[c]{@{}l@{}}Corruzione dei \\ dati di salvataggio\end{tabular} & \begin{tabular}[c]{@{}l@{}}Perdita di dati\\ a causa di un\\ problema di\\ hardware o software\end{tabular} & Bassa                & Bassa                 & \begin{tabular}[c]{@{}l@{}}Implementare\\ backup automatici\end{tabular}                                  \\
                                                                                &                                                                                                             & \multicolumn{1}{l}{} & \multicolumn{1}{l}{}  &                                                                                                           \\
    Bug del codice                                                                & \begin{tabular}[c]{@{}l@{}}Presenza di bug nel\\ codice di logica\\ del gioco\end{tabular}                  & Alta                 & Alta                  & \begin{tabular}[c]{@{}l@{}}Implementare\\ test esaustivi\\ durante l’intero\\ sviluppo\end{tabular}       \\ \bottomrule
    \end{tabular}
...
\begin{enumerate}
    ...
    \item \textit{Logic Layer (Controller)}: Gestisce il flusso dell'applicazione e le regole di business. Coordina le interazioni tra i dati e la visualizzazione. Classi principali: \textit{QuizManager}, \textit{QuizCRUD}, \textit{SceneLoader}.
    \item \textit{Presentation Layer (View)}: Responsabile della visualizzazione dei dati a schermo. Classi principali: \textit{AnswerScript} e \textit{PanelManager}.
\end{enumerate}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=15.5cm]{images/class_diagram1.drawio.png}
    \caption{Diagramma delle classi del serious game}
\end{figure}

\newpage
\section{Testing}
Per assicurarsi il corretto funzionamento di \textcolor{red}{NOME} è stato indispensabile avere una fase di testing precisa e ben definita. Queste specifiche sono state raccolte nel documento \textit{Software Test Plan}.
...
L’ultimo passaggio della fase di testing è stato confermare il corretto funzionamento di \textcolor{red}{NOME} al di fuori dell’ambiente di sviluppo.
