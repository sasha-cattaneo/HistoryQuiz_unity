...
Il \textit{serious game} Quiz me! consiste in un programma \textit{cross-platform}, sia per computer sia per dispositivi mobili, per la gestione e l’esecuzione di quiz.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm]{images/screenshotStartPage.png}
...
\begin{itemize}
    ...
    \item La modalità \textit{Daily}: nella quale l’utente può giocare ad un quiz giornaliero, creato automaticamente in modo da avere un quiz diverso ogni giorno.
        ...
        L'utente ha una sola possibilità per completare questo quiz. Quando riesce a completarlo con successo, cioè con almeno 80\% del punteggio massimo, la sua \textit{streak} cresce di uno.
        La \textit{streak} è un contatore dei giorni consecutivi in cui il quiz giornaliero è stato completato con successo.
...
Altri vantaggi di questo formato sono la leggerezza, che facilita le operazioni di condivisione dei quiz tra i docenti e i loro studenti, e la possibilità di usare degli strumenti già sviluppati per verificare l'integrità del file e per garantire che si tratti di un quiz valido.
...
L'ultimo aspetto pedagogico del gioco sviluppato è la \textit{Daily Challenge}. Questa modalità riprende l'idea dei giochi puzzle online, come ad esempio \textit{Wordle}[21], che propongono un puzzle diverso ogni giorno ed invitano l'utente a tornare ogni giorno grazie al mantenimento della \textit{streak}.
