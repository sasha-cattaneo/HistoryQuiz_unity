...
public class QuizCRUD : MonoBehaviour
{
    ...
    public void importQuiz()
    {
        ...
        var extensions = new[] {
            new ExtensionFilter("Quiz File", "json"),
            new ExtensionFilter("Tutti i file", "*" ),
        };
        var paths = StandaloneFileBrowser.OpenFilePanel("Apri File", "", extensions, false);
        //Debug.Log("Selected files: [" + string.Join(", ", paths) + "]");
        if (paths.Length > 0 && !string.IsNullOrEmpty(paths[0]))
        {
            foreach (var path in paths)
            {
                ...
                else
                {
                    ...
                    ShowMessage(fileName.Replace(".json", "") + " importato con successo", Color.green);
                    addQuizPanel(fileName.Replace(".json", ""));
                }
            }
    ...
    public void exportQuiz(string quizName)
    {
        ...
        var path = StandaloneFileBrowser.SaveFilePanel("Salva File", "", quizName + ".json", extensions);
